...
public class CreadorUnidadesVoladorasMouseListener extends MouseAdapter {
	//private VentanaMapa ventanaMapa;
	private AlgoCraft juego;
	private Posicion posicionCeldaPresionada;
	
    public CreadorUnidadesVoladorasMouseListener(VentanaMapa ventanaMapa, AlgoCraft juego, Posicion posicionCeldaPresionada) {
	...
	public void mouseReleased(MouseEvent arg0) {		
		...
		try {
			if (this.juego.dameElJugadorDelTurno().dameRaza().esRazaProtoss()){
				this.juego.agregarConstruccion(this.juego.dameElJugadorDelTurno(),TipoConstruccion.creadorUnidadesVoladoras,posicionCeldaPresionada);
			}
			else{
				this.juego.agregarConstruccion(this.juego.dameElJugadorDelTurno(),TipoConstruccion.creadorUnidadesEspecialesYVoladoras,posicionCeldaPresionada);
			}
